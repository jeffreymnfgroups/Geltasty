        ...
        <div className='container'>
          <div className='flex flex-col justify-center items-center gap-10 md:gap-20'>
            ...
            <div className='grid md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-8'>
              {creativeMindList?.map((item: unknown, index: number) => {
                const creativeMind = item as { image: string; name: string; position: string; twitterLink: string; linkedinLink: string }
                return (
                  <motion.div {...bottomAnimation(index)} key={index}>
                    <SingleCreativeMind key={index} creativemind={creativeMind}/>
                  </motion.div>
                )
              })}
            </div>
